...
function createBasicMap(e, t, a, o) {
    ...
    if (document.getElementById(e)) {
        ...
        L.tileLayer("https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png", {
            ...
            icon: r
        }).addTo(p).bindPopup("<div class='p-4'><h5>Info Window Content</h5><p class='mb-0'>Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo.</p></div>", {
            minwidth: 200,
            maxWidth: 400,
    ...
    function c(e, a) {
        a.on({
            ...
            if (e.image) {
                ...
                var r = '<div class="' + i + '" style="background-image: url(\'' + t.imgBasePath + e.image + "')\"></div>"
            } else r = '<div class="image"></div>';
            if (e.address) var s = '<p class="text-muted mb-1"><i class="fa fa-map-marker fa-fw text-dark mr-2"></i>' + e.address + "</p>";
            else s = "";
            if (e.email) var n = '<p class="text-muted mb-1"><i class="fa fa-envelope-open fa-fw text-dark mr-2"></i><a href="mailto:' + e.email + '" class="text-muted">' + e.email + "</a></p>";
            else n = "";
            if (e.phone) var p = '<p class="text-muted mb-1"><i class="fa fa-phone fa-fw text-dark mr-2"></i>' + e.phone + "</p>";
            else p = "";
            if (e.stars) {
                for (var c = '<div class="text-xs">', m = 1; m <= 5; m++) m <= e.stars ? c += "<i class='fa fa-star text-warning'></i>" : c += "<i class='fa fa-star text-gray-300'></i>";
                c += "</div>"
            } else c = "";
            if (e.url) e.url, e.url;
            else "";
            var l = "";
            "venue" == t.mapPopupType ? l = '<div class="popup-venue">' + r + '<div class="text">' + a + o + s + n + p + "</div></div>" : "rental" == t.mapPopupType && (l = '<div class="popup-rental">' + r + '<div class="text">' + a + c + "</div></div>");
            return l
        }(e.properties), {
            minwidth: 200,
            maxWidth: 600,
            className: "map-custom-popup"
        }), t.useTextIcon && a.bindTooltip('<div id="customTooltip-' + e.properties.id + '">$' + e.properties.price + "</div>", {
            direction: "top",
            permanent: !0,
            opacity: 1,
            interactive: !0,
